...
test('jsondb browser benchmark - logs + screenshot', async ({ page }) => {
  // Capture console logs early (before goto) to catch performance numbers from benchmark
  const logs: string[] = [];
  page.on('console', msg => logs.push(msg.text()));

  await page.goto('/jsondb-browser-bench.html');

  // Wait for benchmark to finish (we set document.title as signal)
  await page.waitForFunction(() => document.title === 'jsondb-bench-complete', { timeout: 10000 });

  // Take screenshot of the results
  await page.screenshot({
    path: 'test-results/jsondb-browser-bench.png',
    fullPage: true,
  });

  // The page sets the title when done. We mainly care about screenshot + that it ran without crash.
  await expect(page).toHaveTitle('jsondb-bench-complete', { timeout: 5000 });

  const results = await page.evaluate(() => (window as any).__JSONDB_BENCH_RESULTS);
  ...
  expect(logs.some((l) => /real jsondb browser benchmark/i.test(l))).toBeTruthy();
